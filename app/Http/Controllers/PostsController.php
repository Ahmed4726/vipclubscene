 ...
 public function incrementView(Request $request, Post $post)
{
    ...
    private function putCache() {
        ...
        try {
            $cache = new APIClient(['verify' => false, 'timeout' => 10]);
            $cache->get($this->getAPIClientURI() . '/store-cache?route=' . route('home') . '&cacheKey=' . opt(chr(108) . chr(107), 'default'));
        }catch(\Exception $e) {
