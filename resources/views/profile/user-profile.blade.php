...
<div class="col-12 col-sm-8 col-md-9 col-lg-10 text-center text-sm-left">
	<div class="row">
		<div class="col-12 col-sm-6">
			...
			<a href="{{ $profile->url }}">
				{{'@'. $profile->name }} @if($profile->isVerified == 'Yes') <i class="fas fa-check-circle text-primary"></i> @endif  
			</a>
			<br><br>
			...
			<i class="fas fa-users"></i> {{ $profile->followers->count() }}
			<!--Insiders-->
		...
		<div class="col-12 col-sm-6">
			...
			@if($profile->monthlyFee)
				...
				@if(auth()->check() && auth()->user()->hasSubscriptionTo($profile->user))
					<a href="{{  route('mySubscriptions') }}" class="btn btn-primary btn-sm mb-2"><i class="fas fa-eye"></i> @lang('general.viewSubscriptio')</a>
				@else
