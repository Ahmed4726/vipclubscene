@if($post->status == 'visible' || $post->status == 'null')
<div class="card mb-4" data-post-id="{{ $post->id }}">
		...
		<div class="col-9 col-sm-9 col-md-9 col-lg-10">
			...
			<div class="float-left">
				...
				<span class="d-block d-sm-block d-md-none">
					...
					<small>
                        <i class="fas fa-clock mr-1"></i> {{ \Carbon\Carbon::parse($post->created_at)->format('g:i:s A - m d, Y') }}
					</small>
					<br>
					 @if($post->media_type == 'Video')
                        <span class="text-muted">
                            <small>
                                 <i class="fas fa-eye mr-1"></i> <span id="views-count-{{ $post->id }}">{{ $post->views }}</span> Views
                            </small>
                        </span>
                    @endif
				</span>
    ...
    @if($post->is_Posted == 1)
        ...
        <a href="{{ $post->profile_handle }}" class="post-handle-url">
            {{ '@' . $post->profile_handle }}
        </a>
    @endif
</div>
...
<div class="pl-4 pr-4 pt-0 pb-2 text-333">
    <div class="text-content" id="text-content-{!! $post->id !!}">
    ...
    <button class="read-more" id="read-more-{!! $post->id !!}" onclick="toggleText({!! $post->id !!})" style="display: none;">Read More</button>
</div>
   ...
   <i class="fas fa-heart"></i> <span class="post-likes-count" data-id="{{ $post->id }}">{{ $post->likes->count() }}</span>
		...
		@include('tips.tip-form')
			...
			<h4 class="d-inline">
            <form action="{{ route('repostPost', ['post' => $post->id]) }}" method="POST" style="display:inline;">
                    @csrf
 ...
 <div class="appendComments d-none" data-id="{{ $post->id }}">
    {!! clean(turnLinksIntoAtags(
        preg_replace_callback(
            '/(@\w+|#\w+)/',
            function($matches) {
